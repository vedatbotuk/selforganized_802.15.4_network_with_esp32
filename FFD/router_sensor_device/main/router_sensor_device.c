    ...
    while (1)
    {
        if (connected)
        {
            ...
            if (dht_read_float_data(SENSOR_TYPE, CONFIG_EXAMPLE_DATA_GPIO, &humidity, &temperature) == ESP_OK)
            {
                ...
                zb_update_temp((int16_t)(temperature * 100), ROUTER_SENSOR_DEVICE_ENDPOINT);
                zb_update_hum((uint16_t)(humidity * 100), ROUTER_SENSOR_DEVICE_ENDPOINT);
            }
            else
            {
            ...
            if (get_battery_level(&battery_level, &battery_voltage) == ESP_OK) {
                ...
                zb_update_battery_level((uint8_t)(2 * battery_level), (uint8_t)(battery_voltage), ROUTER_SENSOR_DEVICE_ENDPOINT);
            } else {
